...
const METRICS_KEY = "evaluation_metrics";
const activeRuns = new Map();

function initEvaluationMetricsStorage() {
  try {
    chrome.storage.local.get([METRICS_KEY], (items) => {
      const arr = items && Array.isArray(items[METRICS_KEY]) ? items[METRICS_KEY] : null;
      if (!arr) chrome.storage.local.set({ [METRICS_KEY]: [] }, () => void 0);
    });
  } catch (_) {}
}
...
function appendMetric(entry) {
  try {
    chrome.storage.local.get([METRICS_KEY], (items) => {
      ...
      arr.push(entry);
      chrome.storage.local.set({ [METRICS_KEY]: arr }, () => void 0);
    });
  } catch (_) {}
}

function startRun(batchSize) {
...
function ackRender(runId) {
  const run = activeRuns.get(runId);
  if (!run) return;
  run.acks += 1;
  if (run.acks >= run.batchSize) finalizeRun(runId);
}

function addQueueSample(runId, enqueuedAt, startedAt, queueLenAtEnqueue) {
  const run = activeRuns.get(runId);
  if (!run) return;
  if (typeof enqueuedAt === "number" && typeof startedAt === "number") {
    run.queueWaitSumMs += Math.max(0, startedAt - enqueuedAt);
    run.queueSamples += 1;
  }
  if (typeof queueLenAtEnqueue === "number") {
    run.queueLengthMax = Math.max(run.queueLengthMax, queueLenAtEnqueue);
  }
}

function markCacheHit(runId) {
  const run = activeRuns.get(runId);
  if (!run) return;
  run.cacheHitAny = true;
}

function addTokens(runId, tokens) {
  const run = activeRuns.get(runId);
  if (!run) return;
  const t = Number(tokens);
  if (!isNaN(t) && t > 0) run.tokensTotal += t;
}

function markError(runId, msg) {
  const run = activeRuns.get(runId);
  if (!run) return;
  run.errors.push(String(msg || ""));
}

function finalizeRun(runId) {
  const run = activeRuns.get(runId);
  if (!run) return;
  const latencyMs = Math.max(0, performance.now() - run.startPerf);
  const throughput = run.batchSize > 0 ? (run.batchSize / (latencyMs / 1000 || 1e-9)) : 0;
  const queueWaitAvg = run.queueSamples > 0 ? (run.queueWaitSumMs / run.queueSamples) : 0;
  const entry = {
    runId,
    ...
    tokensPerEmail: run.batchSize ? (run.tokensTotal / run.batchSize) : 0,
    cacheHit: !!run.cacheHitAny,
    queueWaitMs: queueWaitAvg,
    queueLength: run.queueLengthMax,
    success: run.errors.length === 0,
    error: run.errors.length ? (run.errors.length === 1 ? run.errors[0] : `${run.errors.length} errors`) : null,
  };
  appendMetric(entry);
...
async function callGroq(promptText) {
  ...
  return { text: cleanSummaryOutput(String(text).trim()), usage: (data && data.usage) || null };
}
  ...
  while (queue.length > 0) {
    ...
    try {
      ...
      if (idHashEntry && idHashEntry.summary) {
        ...
        addQueueSample(item.runId, item.enqueuedAt, performance.now(), item.queueLenAtEnqueue);
        markCacheHit(item.runId);
      ...
      addQueueSample(item.runId, item.enqueuedAt, performance.now(), item.queueLenAtEnqueue);
      const gr = await callGroq(item.emailText);
      const summary = gr && gr.text ? gr.text : "";
      const totalTokens = gr && gr.usage && typeof gr.usage.total_tokens === "number" ? gr.usage.total_tokens : 0;
      addTokens(item.runId, totalTokens);
      ...
      if (item && item.runId) markError(item.runId, (e && e.message) || String(e));
    }
    await sleep(QUEUE_DELAY_MS);
  }
  processing = false;
...
async function handleMultiSummarizeRequest(items, tabId, runId) {
    ...
    try {
      ...
      if (it.kind === "message") {
        ...
        const qItem = {
          ...
          queueLenAtEnqueue: queue.length,
        };
        queue.push(qItem);
        enqueued.add(key);
      ...
      markError(runId, (e && e.message) || String(e));
    }
  }
  processQueue();
}
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  try {
    if (msg && msg.type === "mailmind_multi_summarize_request") {
      ...
      const runId = startRun(Array.isArray(msg.items) ? msg.items.length : 0);
      handleMultiSummarizeRequest(Array.isArray(msg.items) ? msg.items : [], tabId, runId);
      sendResponse({ ok: true, runId });
      return true;
    }
    if (msg && msg.type === "mailmind_metrics_render_ack") {
      if (msg.runId) ackRender(msg.runId);
      sendResponse({ ok: true });
      return true;
    }
    if (msg && msg.type === "mailmind_export_metrics") {
      try {
        chrome.storage.local.get([METRICS_KEY], (items) => {
          const arr = items && Array.isArray(items[METRICS_KEY]) ? items[METRICS_KEY] : [];
        ...
        try {
          ...
          if (headerId) {
            ...
            if (idHashCheck.cached) {
              ...
              sendToTab(tabId, { type: "mailmind_single_result", mode: "summary", summary: idHashCheck.summary, source: "cache-id", runId });
              markCacheHit(runId);
          ...
          const bodyHash = await sha256(bodyText);
          {
            ...
            if (hashCache[bodyHash] && hashCache[bodyHash].summary) {
              ...
              sendToTab(tabId, { type: "mailmind_single_result", mode: "summary", summary: hashCache[bodyHash].summary, source: "cache-body", runId });
              markCacheHit(runId);
              sendResponse({ ok: true, cached: true });
              return;
            }
          ...
          // 3) Call Groq
          const gr = await callGroq(bodyText);
          const summary = gr && gr.text ? gr.text : "";
          const totalTokens = gr && gr.usage && typeof gr.usage.total_tokens === "number" ? gr.usage.total_tokens : 0;
          addTokens(runId, totalTokens);
          ...
          await setCaches(idCache, hashCache, idHashCache);
          sendToTab(tabId, { type: "mailmind_single_result", mode: "summary", summary, source: "groq", runId });
          sendResponse({ ok: true });
        } catch (e) {
          sendToTab(tabId, { type: "mailmind_single_error", mode: "summary", error: (e && e.message) || String(e), runId });
          if (typeof runId === 'string') markError(runId, (e && e.message) || String(e));
          sendResponse({ ok: false, error: (e && e.message) || String(e) });
        }
      })();
      return true;
